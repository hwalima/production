@extends('layouts.app')
@section('title', 'Mining Departments')
...
<div style="max-width:800px;">
    ...
    <div class="form-card" style="margin-bottom:20px;">
        <p style="font-size:.72rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#fcb913;margin-bottom:12px;">● Editing Department</p>
        <form method="POST" action="{{ route('mining-departments.update', $editing) }}" style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;">
            @csrf @method('PUT')
            <div style="flex:1;min-width:180px;">
                <label class="fc-label">Department Name <span style="color:#ef4444;">*</span></label>
                <input type="text" name="name" value="{{ old('name', $editing->name) }}"
                       class="fc-input" placeholder="e.g. Open Cast, Underground, Mill" required>
            ...
            <div style="display:flex;align-items:center;gap:6px;padding-bottom:2px;">
                ...
                <label for="edit_active" class="fc-label" style="margin:0;">Active</label>
            </div>
            <div style="display:flex;gap:8px;">
                <button type="submit" class="btn-add" style="padding:9px 18px;font-size:.82rem;">Save</button>
                <a href="{{ route('mining-departments.index') }}" class="btn-cancel">Cancel</a>
            </div>
        </form>
    </div>
    @else
    <div class="form-card" style="margin-bottom:20px;">
        <p style="font-size:.72rem;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#fcb913;margin-bottom:12px;">● Add New Department</p>
        <form method="POST" action="{{ route('mining-departments.store') }}" style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;">
            @csrf
            <div style="flex:1;min-width:180px;">
                <label class="fc-label">Department Name <span style="color:#ef4444;">*</span></label>
            ...
            <div>
                <button type="submit" class="btn-add" style="padding:9px 18px;font-size:.82rem;">+ Add Department</button>
            </div>
        </form>
    </div>
    @endisset
            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="color:#9ca3af;font-size:.82rem;">{{ $dept->description ?: '—' }}</td>
                        <td class="td-c">
                            <form method="POST" action="{{ route('mining-departments.toggle', $dept) }}" style="display:inline;">
                                @csrf @method('PATCH')
                        ...
                        <td class="td-c">
                            <div class="act-group">
                                <a href="{{ route('mining-departments.edit', $dept) }}" class="act-btn act-edit" title="Edit">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                                </a>
                                <form method="POST" action="{{ route('mining-departments.destroy', $dept) }}" style="display:contents"
                                      onsubmit="event.preventDefault();confirmDelete('Delete department: {{ addslashes($dept->name) }}? This cannot be undone.',this)">
                                    @csrf @method('DELETE')
                    ...
                    @empty
                    <tr class="empty-row"><td colspan="5">No departments configured yet. Add your first department above.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
