            ...
            <table style="width:100%;border-collapse:collapse;min-width:680px;">
                ...
                <tbody>
                    ...
                    <tr style="{{ $loop->even ? 'background:rgba(255,255,255,.02);' : '' }}">
                        <td style="padding:8px 12px;font-size:.82rem;font-weight:600;color:var(--text);">
                            {{ $label }}
                        </td>
                        @foreach($departments as $dept)
                        @php
                            $raw = $indicators[$dept]?->{$field} ?? 0;
                            $val = (float)$raw > 0 ? $raw : '';
                            $oldVal = old("indicators.{$dept}.{$field}", $val);
                        @endphp
                        <td style="padding:6px 8px;text-align:center;">
                            <input type="number"
                                   name="indicators[{{ $dept }}][{{ $field }}]"
                                   value="{{ $oldVal }}"
                                   min="0" step="0.01"
                                   placeholder="—"
