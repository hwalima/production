@extends('layouts.app')
@section('title', 'Add Chemical Inventory')
@section('page-title', 'Chemical Inventory')
@section('content')
<div style="max-width:640px;">
    <div class="page-header">
        <h1 class="page-title">Add Chemical Inventory</h1>
        ...
        <form action="{{ route('chemicals.store') }}" method="POST">
            ...
            <div class="fc-grid" style="margin-bottom:14px;">
                <div>
                    ...
                    <input type="number" name="sodium_cyanide" step="0.01" class="fc-input" value="{{ old('sodium_cyanide') }}" required>
                    @error('sodium_cyanide')<p class="fc-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="fc-label">Lime (kg)</label>
                    <input type="number" name="lime" step="0.01" class="fc-input" value="{{ old('lime') }}" required>
                    @error('lime')<p class="fc-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="fc-label">Caustic Soda (kg)</label>
                    <input type="number" name="caustic_soda" step="0.01" class="fc-input" value="{{ old('caustic_soda') }}" required>
                    @error('caustic_soda')<p class="fc-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="fc-label">Iodised Salt (kg)</label>
                    <input type="number" name="iodised_salt" step="0.01" class="fc-input" value="{{ old('iodised_salt') }}" required>
                    @error('iodised_salt')<p class="fc-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="fc-label">Mercury (g)</label>
                    <input type="number" name="mercury" step="0.01" class="fc-input" value="{{ old('mercury') }}" required>
                    @error('mercury')<p class="fc-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="fc-label">Steel Balls (kg)</label>
                    <input type="number" name="steel_balls" step="0.01" class="fc-input" value="{{ old('steel_balls') }}" required>
                    @error('steel_balls')<p class="fc-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="fc-label">Hydrogen Peroxide (L)</label>
                    <input type="number" name="hydrogen_peroxide" step="0.01" class="fc-input" value="{{ old('hydrogen_peroxide') }}" required>
                    @error('hydrogen_peroxide')<p class="fc-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="fc-label">Borax (kg)</label>
                    <input type="number" name="borax" step="0.01" class="fc-input" value="{{ old('borax') }}" required>
                    @error('borax')<p class="fc-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="fc-label">Nitric Acid (L)</label>
                    <input type="number" name="nitric_acid" step="0.01" class="fc-input" value="{{ old('nitric_acid') }}" required>
                    @error('nitric_acid')<p class="fc-error">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="fc-label">Sulphuric Acid (L)</label>
                    <input type="number" name="sulphuric_acid" step="0.01" class="fc-input" value="{{ old('sulphuric_acid') }}" required>
